...
class RemoveCommand extends clipanion_1.Command {
    constructor() {
        ...
        this.outDir = clipanion_1.Option.String("--outDir", { required: true });
    }
    get bfgPath() {
        return path_1.default.join(this.outDir, "bfg.jar");
    }
    get repoPath() {
        return path_1.default.join(this.outDir, REPO_DIR);
    }
    get blobIdsPath() {
        return path_1.default.join(this.outDir, "blobids.txt");
    }
    async execute() {
        if (!fs_1.existsSync(this.outDir)) {
            fs_1.mkdirSync(this.outDir);
        }
        ...
        console.log();
        console.log(`Go to ${this.repoPath} and inspect the changes then push it, if desired. Remember that it's a mirror, so the files will not be visible. Git commands should work though.`);
    }
    async downloadBfg() {
